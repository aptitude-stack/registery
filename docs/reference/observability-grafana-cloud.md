...
| Metrics | OpenTelemetry Meter API instruments declared in `app/observability/metrics.py` (`aptitude_http_requests_total`, `aptitude_http_request_duration_seconds`, `aptitude_registry_operation_total`, `aptitude_registry_operation_duration_seconds`, `aptitude_readiness_status`) | Recorded by FastAPI request middleware and the readiness probe. Exported every minute via `PeriodicExportingMetricReader`. |
