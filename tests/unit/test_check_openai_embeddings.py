...
@pytest.mark.unit
def test_openai_embeddings_deploy_check_fails_loudly_when_key_is_absent() -> None:
    settings = Settings(
        _env_file=None,
        DATABASE_URL=DATABASE_URL,
    ...
    assert result.message == "OpenAI embedding check failed: OPENAI_API_KEY is required."


@pytest.mark.unit
def test_openai_embeddings_deploy_check_reports_provider_errors_without_key() -> None:
